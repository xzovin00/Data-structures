...
int stackEmpty ( const tStack* s ) {
	...
	return !(1+s->top); 		// 1-1 je nula neboli FALSE znegovano na TRUE, ostatni moznosti jsou > 0 takze TRUE znegovane na FALSE
...
int stackFull ( const tStack* s ) {
	...
	return !(s->top+1-STACK_SIZE); // +1 protoze top zacina od nuly (pro neprazdny stack)
...
void stackTop ( const tStack* s, char* c ) {
	...
	if (stackEmpty(s))
		...
		*c=s->arr[s->top]; 	// do c se ulozi hodnota s->arr na pozici stack top
...
void stackPop ( tStack* s ) {
	...
	if (!stackEmpty(s)) 		
		s->top--;	//snizime ukazatel o jedna, data co byla ulozene na dane pozici budou prepsana pri prisrtim stackPush
...
void stackPush ( tStack* s, char c ) {
	...
	if (stackFull(s))
		...
		s->arr[++s->top]=c;		// s->top inkrementujeme a do s->arr na pozici s->top ulozime c
