...
**/
	...
	tBTNodePtr new =*RootPtr;

	while(new->Cont != Content) {
		if(Content < new->Cont){
			if(new->LPtr == NULL ){
				new->LPtr = malloc(sizeof(struct tBTNode));
				if ( !new )
					return;
				new->LPtr->Cont = Content;
				new->LPtr->LPtr = new->LPtr->RPtr = NULL;
				return;
			}else
				new = new->LPtr;
			...
			if(new->RPtr == NULL ){
				new->RPtr = malloc(sizeof(struct tBTNode));
				if ( !new )
					return;
				new->RPtr->Cont = Content;
				new->RPtr->LPtr = new->RPtr->RPtr = NULL;
				return;
			}else
				new = new->RPtr;
		}
...
**/
	...
	while(!SEmptyP(&Stack)){
		...
		tBTNodePtr delete_me = STopPopP(&Stack);
		if(!delete_me)
			continue;

		SPushP(&Stack, delete_me->LPtr);
		SPushP(&Stack, delete_me->RPtr);
